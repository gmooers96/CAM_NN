...
def PlotAllFigs_nn(path_nn_stage2, datadir, training_expt, est_str, trainfile, testfile, do_nn, figpath, input_vert_vars, output_vert_vars, input_vert_dim, output_vert_dim, zdim,
                n_trn_exs=None,
    ...
    f_scl, opred_scl, otrue_scl, f, opred, otrue = \
        ml_load.get_f_o_pred_true(est_str, datadir, training_expt, trainfile, testfile, zdim, max_z=max(z), input_vert_vars=input_vert_vars,
    ...
    make_yz_plots(figpath, f_ppi, o_ppi, f_pp, o_pp, est_eval, y, z, p, rho, input_vert_vars, output_vert_vars,
                  ...
                  testfile, n_trn_exs, rain_only, no_cos, use_rh, wind_input=wind_input,
                  scale_per_column=scale_per_column,
...
def plot_importance(figpath, est_eval, z, p, rho, use_rh, wind_input):
    ...
    fig.savefig(figpath + 'feature_importance.png', bbox_inches='tight')
    plt.close()
...
def plot_importance_permute(figpath, f_scl, otrue_scl, est_eval, z, p, rho, use_rh, wind_input):
    ...
    ax.spines['top'].set_color('none')
    plt.tight_layout()  # avoid overlap
    fig.savefig(figpath + 'feature_importance_permute.png', bbox_inches='tight')
    plt.close()
...
def plot_importance_precip_permute(figpath, f_scl, otrue_scl, est_eval, z, p, rho, f_ppi, f_pp, o_ppi, o_pp, use_rh,
    ...
    ax.spines['top'].set_color('none')
    plt.tight_layout()  # avoid overlap
    fig.savefig(figpath + 'feature_importance_precip_permute.png', bbox_inches='tight')
    plt.close()
...
def plot_linear_response(figpath, f, est_eval, z, p, rho, f_ppi, f_pp, o_ppi, o_pp, use_rh=False, wind_input=False):
    ...
    ax.spines['top'].set_color('none')
    plt.tight_layout()  # avoid overlap
    fig.savefig(figpath + 'linear_response.png', bbof_inches='tight')
    plt.close()
    ...
    plt.tight_layout()  # avoid overlap
    fig.savefig(figpath + 'linear_response_feature_importance.png', bbox_inches='tight')
    plt.close()
...
def make_yz_plots(figpath, f_ppi, o_ppi, f_pp, o_pp, est_eval, y, z, p, rho, input_vert_vars, output_vert_vars,
    ...
    for out_feature, out_dim in zip(output_vert_vars, output_vert_dim):
        if (out_dim > 1 and out_feature != 'Tout' and out_feature != 'qout'):
            ...
            plt.tight_layout()  # avoid overlap
            fig.savefig(figpath + 'yz_' + out_feature + '_truemean_bias.png', bbox_inches='tight')
            plt.close()
            ...
            plt.tight_layout()  # avoid overlap
            fig.savefig(figpath + 'yz_' + out_feature + '_var_rmse.png', bbox_inches='tight')
            plt.close()
            ...
            plt.tight_layout()  # avoid overlap
            fig.savefig(figpath + 'yz_' + out_feature + '_r_Rsq.png', bbox_inches='tight')
            plt.close()
            ...
            ax.spines['top'].set_color('none')
            plt.tight_layout()  # avoid overlap
            plt.savefig(figpath + out_feature + 'Truemean_bias_y.png', bbox_inches='tight')
            plt.close()
            ...
            ax.spines['top'].set_color('none')
            plt.tight_layout()  # avoid overlap
            plt.savefig(figpath + out_feature + 'var_rmse_y.png', bbox_inches='tight')
            plt.close()
        ...
        ax.spines['top'].set_color('none')
        plt.tight_layout()  # avoid overlap
        plt.savefig(figpath + 'precip_mean.png', bbox_inches='tight')
        plt.close()
        ...
        ax.spines['top'].set_color('none')
        plt.tight_layout()  # avoid overlap
        plt.savefig(figpath + 'precip_extremes.png', bbox_inches='tight')
        plt.close()
    ...
    if 'Tout_mean' in output_stat_dict.keys():
        ...
        plt.tight_layout()  # avoid overlap
        fig.savefig(figpath + 'yz_truemean.png', bbox_inches='tight')
        plt.close()
        # Bias from true mean
        ...
        plt.tight_layout()  # avoid overlap
        fig.savefig(figpath + 'yz_bias.png', bbox_inches='tight')
        plt.close()
        ...
        plt.tight_layout()  # avoid overlap
        fig.savefig(figpath + 'yz_rmse.png', bbox_inches='tight')
        plt.close()
        ...
        #   plt.tight_layout() # avoid overlap
        fig.savefig(figpath + 'yz_corrcoeff.png', bbox_inches='tight')
        plt.close()
        ...
        ax1.set_title('(a) Temperature tendency', fontsize=8)
        ax2.set_title('(b) Humidity tendency', fontsize=8)
        fig.savefig(figpath + 'yz_Rsq.png', bbox_inches='tight')
        plt.close()
...
def plot_means_stds_generalized(y3_true, y3_pred, p, figpath, output_vert_vars, output_vert_dim):
    ...
    for key, dim in zip(output_vert_vars, output_vert_dim):
        ...
        if dim > 1:
            ...
            dum_i = dum_i + 2
        plt.tight_layout()  # avoid overlap
    fig.savefig(figpath + 'means_stds.png', bbox_inches='tight')
    plt.close()
...
def plot_error_stats(y3_true, y3_pred, p, figpath, output_vert_vars, output_vert_dim):
    fig = plt.figure(figsize=(12.0, 9)) #6.0, 4.5
    true_out_dict = ml_load.unpack_list(y3_true, output_vert_vars, output_vert_dim)
    pred_out_dict = ml_load.unpack_list(y3_pred, output_vert_vars, output_vert_dim)
    ...
    plt.tight_layout()  # avoid overlap
    fig.savefig(figpath + 'error_stats.png', bbox_inches='tight')
    plt.close()
...
def plot_energy_conservation(f, o_true, o_pred, z, p, rho, figpath, input_vert_vars, input_vert_dim, output_vert_vars,
    ...
    plt.legend(frameon=False)
    plt.tight_layout()  # avoid overlap
    fig.savefig(figpath + 'energy_conservation.png', bbox_inches='tight')
    plt.close()
